        ...
        <div className="mt-3 grid grid-cols-2 gap-2 min-[520px]:grid-cols-4">
          <div className={`rounded-md border px-2.5 py-2 ${statusClass(explanation.status)}`}>
            ...
            <p className="mt-1 text-sm font-bold tabular-nums">
              {formatRegressionValue(explanation.scoreDelta)}
            </p>
            <p className="text-[10px] font-semibold tabular-nums">
              {formatRegressionDelta(explanation.scoreDelta)}
            </p>
          </div>
        ...
        <div className="mt-3 rounded-md border border-perf-border bg-perf-bg/45 p-3">
          ...
          <ul className="mt-2 space-y-1.5">
            {explanation.likelyCauses.map((cause) => (
              <li key={cause} className="text-xs leading-relaxed text-perf-text">
                {cause}
              </li>
            ))}
          </ul>
            ...
            <div className="rounded-md border border-perf-border bg-perf-bg/45 p-3">
              ...
              <div className="mt-2 space-y-2">
                {explanation.newIssues.slice(0, 3).map((issue) => (
                  <div key={`${issue.category}-${issue.description}`} className="text-xs leading-relaxed">
                    <span className="font-semibold text-perf-text">{issue.category}</span>
                    <span className="ml-1 text-perf-muted">{issue.description}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-md border border-perf-border bg-perf-bg/45 p-3">
              ...
              <div className="mt-2 space-y-2">
                {explanation.newLargeResources.slice(0, 3).map((resource) => (
                  <div key={resource.name} className="text-xs leading-relaxed">
                    <span className="font-semibold text-perf-text">{resource.type}</span>
                    <span className="ml-1 text-perf-muted">
                      {formatRegressionValue({ label: 'Size', before: null, after: resource.size, delta: null, unit: 'bytes', worsened: false })} · {shortResourceName(resource.name)}
                    </span>
                  </div>
                ))}
              </div>
